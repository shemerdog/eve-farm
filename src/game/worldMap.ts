...
export const isPurchased = (coord: TileCoord, purchasedCoords: TileCoord[]): boolean =>
    purchasedCoords.some((c) => coordsEqual(c, coord))
...
export const isAdjacentToUnlocked = (coord: TileCoord, purchasedCoords: TileCoord[]): boolean => {
...
export const applyMomentumFrame = (
    ...
    const done =
        Math.abs(nextVelX) < MOMENTUM_STOP_THRESHOLD && Math.abs(nextVelY) < MOMENTUM_STOP_THRESHOLD
    return {
