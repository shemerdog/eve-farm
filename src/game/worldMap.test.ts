...
describe("getTileType", () => {
  ...
  it("returns locked for tiles adjacent to farm", () => {
    expect(getTileType({ col: FARM_COORD.col + 1, row: FARM_COORD.row })).toBe(
      "locked",
    );
    expect(getTileType({ col: FARM_COORD.col, row: FARM_COORD.row + 1 })).toBe(
      "locked",
    );
  });
...
describe("buildTileGrid", () => {
  ...
  it("all other tiles are locked", () => {
    expect(grid.filter((t) => t.type === "locked")).toHaveLength(
      MAP_COLS * MAP_ROWS - 1,
    );
  });
});
...
describe("applyMomentumFrame", () => {
  it("decays velocity by MOMENTUM_FRICTION each frame", () => {
    const result = applyMomentumFrame(
      { x: -500, y: -500, zoom: 1 },
      10,
      10,
      430,
...
describe("isAdjacentToUnlocked", () => {
  ...
  it("returns true when orthogonally adjacent to a purchased tile", () => {
    ...
    expect(isAdjacentToUnlocked({ col: 4, row: 2 }, [{ col: 3, row: 2 }])).toBe(
      true,
    );
  });
  it("returns false when only diagonally adjacent to a purchased tile", () => {
    expect(isAdjacentToUnlocked({ col: 4, row: 3 }, [{ col: 3, row: 2 }])).toBe(
      false,
    );
  });
...
describe("clampCamera — zoom field", () => {
  it("preserves zoom when in valid range", () => {
    expect(clampCamera({ x: -500, y: -400, zoom: 1.0 }, 430, 700).zoom).toBe(
      1.0,
    );
  });
  it("clamps zoom below MIN_ZOOM up to MIN_ZOOM", () => {
    expect(clampCamera({ x: 0, y: 0, zoom: 0.1 }, 430, 700).zoom).toBe(
      MIN_ZOOM,
    );
  });
  it("clamps zoom above MAX_ZOOM down to MAX_ZOOM", () => {
    expect(clampCamera({ x: 0, y: 0, zoom: 3.0 }, 430, 700).zoom).toBe(
      MAX_ZOOM,
    );
  });
