...
test("buy tile buttons are disabled when player cannot afford", async ({
  page,
}) => {
  await page.goto("/");
...
test("buy tile buttons are enabled when player can afford", async ({
  page,
}) => {
  await page.addInitScript(
    ({ key, value }) => localStorage.setItem(key, value),
...
test("manage decisions panel opens, shows encountered dilemma, and allows toggling", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "נהל החלטות" }),
  ).not.toBeVisible();
});

test("clicking buy tile deducts wheat and unlocks the tile", async ({
  page,
}) => {
  await page.addInitScript(
    ({ key, value }) => localStorage.setItem(key, value),
